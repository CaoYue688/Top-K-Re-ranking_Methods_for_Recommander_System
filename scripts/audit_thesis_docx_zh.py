"""Structural and content audit for the final Chinese thesis DOCX."""
...
def main() -> None:
    ...
    with zipfile.ZipFile(args.docx) as archive:
        ...
        notes = [
            note
            for note in foot_root.xpath(".//w:footnote", namespaces=NS)
            if int(note.get(f"{{{W}}}id")) >= 1
        ]
        if len(notes) != 29:
        ...
        if any(not (image.get("descr") or "").strip() for image in images):
            failures.append("image without alt text")
        tables = root.xpath(".//w:tbl", namespaces=NS)
        if len(tables) != 20:
            failures.append(f"expected 20 tables, got {len(tables)}")
        if any(not table.xpath("./w:tr[1]/w:trPr/w:tblHeader", namespaces=NS) for table in tables):
            failures.append("table without marked header row")
        sections = root.xpath(".//w:sectPr", namespaces=NS)
        formats = [
            (
                section.find("w:pgNumType", namespaces=NS).get(f"{{{W}}}fmt")
                if section.find("w:pgNumType", namespaces=NS) is not None
                else None
            )
    ...
    all_text = body_text + "\n" + table_text
    cjk_chars = len(re.findall(r"[\u3400-\u9fff]", all_text))
    headings = sum(1 for paragraph in doc.paragraphs if paragraph.style.name.startswith("Heading"))
    if cjk_chars < 35_000:
        failures.append(f"unexpectedly short document: {cjk_chars} CJK characters")
    if headings < 90:
    ...
    print(
        "AUDIT OK | "
        f"cjk_chars={cjk_chars} paragraphs={len(doc.paragraphs)} headings={headings} "
